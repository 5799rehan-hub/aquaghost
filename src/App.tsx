...
import { evaluatePhysicsRiskGate } from './utils/sonarAcousticMath';
...
export default function App() {
  ...
  const [preprocessingParams, setPreprocessingParams] = useState<PreprocessingParams>({
    ...
    minHeightThreshold: 0.20, // 20 cm
  });

  // Re-run validation gate whenever altitude H or minHeightThreshold changes
  useEffect(() => {
  ...
  }, [telemetry.auvAltitudeH, preprocessingParams.minHeightThreshold]);
  ...
  const handleExportReport = (format: 'csv' | 'json' | 'geojson') => {
    if (format === 'csv') {
      const header = 'Target_ID,Classification,Status,Latitude,Longitude,Physical_Height_m,Shadow_Length_m,Ground_Range_m,AI_Confidence,Risk_Score,Depth_m\n';
      const rows = targets.map((t) => 
        `"${t.id}","${t.name}","${t.status}",${t.latitude.toFixed(6)},${t.longitude.toFixed(6)},${t.calculatedHeight.toFixed(2)},${t.shadowLengthL.toFixed(2)},${t.groundRangeRg.toFixed(2)},${(t.aiConfidence * 100).toFixed(1)}%,${t.fusedRiskScore},${t.depthMeters.toFixed(1)}`
      ).join('\n');
      const blob = new Blob([header + rows], { type: 'text/csv;charset=utf-8;' });
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = `AQUAGHOST_Detections_${Date.now()}.csv`;
      link.click();
    } else if (format === 'json') {
      const exportJson = {
        system: "AQUAGHOST",
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = `AQUAGHOST_Detections_${Date.now()}.json`;
      link.click();
    } else {
      const geojson = {
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = `AQUAGHOST_Risk_Map_${Date.now()}.geojson`;
      link.click();
    }
  };
  ...
  const flowchartStages = [
    ...
    { num: 7, label: '3D Physics Risk Engine', tab: 'physics3d' as ActiveTabType, highlight: true },
